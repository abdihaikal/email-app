...
const MESSAGE_TYPE: AutomateMessageType = 'anniversary';

@Injectable()
export class AnniversaryMessageService {
  ...
  constructor(
    ...
    this.toSentAt = configService.get<string>(
      'ANNIVERSARY_MESSAGE_TO_SEND_AT',
      '9',
    );
  }
  ...
  private getScheduleDate(user: User) {
  ...
  private isDateExists(user: User): boolean {
    // yyyy-MM-dd = 10 chars
    return user.anniversary_date ? user.anniversary_date.length === 10 : false;
  }
  ...
  private prepareDto(user: User, scheduleDate: LuxonType): Message {
    return this.messageService.repository.create({
      ...
      content: `Hey, ${user.full_name} it's your anniversary! Happy Anniversary!`,
